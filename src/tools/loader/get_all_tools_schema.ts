...
export const getAllToolsSchema = createTool({
  ...
  parameters: z.object({
    toolName: z.string().optional().describe('Optional: Specify a tool name to get its detailed schema. If omitted, returns all available tools.')
  }),
  // 增加第二个参数 context
  handler: async ({ toolName }, context) => {
    ...
    return generateToolMarkdown(targetTools.map(item => ({
      ...item,
      function: {
        ...item.function,
